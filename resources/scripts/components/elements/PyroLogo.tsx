...
const getCSSVariable = (variable: string): string => {
    if (typeof window === 'undefined') return '#fa4e49';
    return getComputedStyle(document.documentElement).getPropertyValue(variable).trim() || '#fa4e49';
};
...
const Logo = ({ className, uniqueId }: { className?: string; uniqueId?: string } = {}) => {
    const customLogoUrl = window.SiteConfiguration?.logoUrl;
    const [brandColor, setBrandColor] = useState('#fa4e49');
    const [gradientColor, setGradientColor] = useState('#F06F53');

    useEffect(() => {
        // Get the brand color from CSS variable
        const color = getCSSVariable('--color-brand');
        setBrandColor(color);

        // Create a lighter variant for the gradient (similar to the original gradient effect)
        const lighter = lightenColor(color, 0.15);
        setGradientColor(lighter);

        // Listen for CSS variable changes
        const observer = new MutationObserver(() => {
            const newColor = getCSSVariable('--color-brand');
            setBrandColor(newColor);
            setGradientColor(lightenColor(newColor, 0.15));
        });
    ...
    return (
        ...
        >
            ...
            <defs>
                ...
                >
                    <stop stopColor={brandColor}></stop>
