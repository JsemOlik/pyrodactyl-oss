...
const QueryInterfaceContainer = () => {
    ...
    const { hasFeature } = useServerEggFeatures();
        ...
        <ServerContentBlock title='Query'>
            ...
            <Can action={'database.read'} matchAny>
                {hasFeature('query-interface') ? (
                            ...
                            <div className='lg:col-span-2 flex flex-col'>
                                <div className='bg-[#ffffff09] border border-[#ffffff11] rounded-lg p-4 flex-1 flex flex-col'>
                                    ...
                                    <p className='text-xs text-white/60 mt-2'>
                                        Only SELECT queries are allowed for security. Queries are limited to 10,000
                                        characters.
                                    </p>
                                </div>
                            </div>
                                                ...
                                                {queryResult.data.map((row, idx) => (
                                                    <tr key={idx} className='border-b border-white/5 hover:bg-white/5'>
                                                        {queryResult.columns.map((col) => (
                                                            <td
                                                                key={col}
                                                                className='p-2 text-white/80 font-mono text-sm'
                                                            >
                    ...
                    <div className='text-center py-8 text-white/60'>Query interface not available for this server.</div>
                )}
            </Can>
