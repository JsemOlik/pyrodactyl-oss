...
class NginxStreamService
{
    ...
    public function generateStreamConfig(ServerSubdomain $subdomain): string
    {
        ...
        // - Listen on 0.0.0.0 (all interfaces) to accept external connections
        // - Proxy to the container's allocation IP and port
        // 
        // If the allocation IP is 0.0.0.0, the container binds to all interfaces,
        // so NGINX on the same host should connect via 127.0.0.1 (localhost).
        // Otherwise, use the allocation IP directly.
        ...
        // If allocation IP is 0.0.0.0, use localhost since the container is on the same host
        // Otherwise, use the allocation IP directly
        ...
        // This is critical for external connections to work
        $config = <<<NGINX
# Auto-generated by Pyrodactyl - Do not edit manually
# Subdomain ID: {$subdomain->id}
# Server ID: {$server->id}
# Generated: {$subdomain->updated_at->toIso8601String()}
# Proxy: 0.0.0.0:{$proxyPort} -> {$containerIp}:{$containerPort}
# Allocation IP: {$allocationIp} (using {$containerIp} for proxy connection)
